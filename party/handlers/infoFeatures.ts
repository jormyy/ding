...
const genericInfoFeatureIds = [
  "earthquake", "tornado", "lightning", "flood", "drought", "plague", "solar-flare", "meteor", "mirror-universe",
...
for (const id of genericInfoFeatureIds) {
  featureHandlers[id] = (_state: ServerGameState, phase: Phase) => genericInfo(id, phase);
}

export function applyModeInfoFeatures(state: ServerGameState, phase: Phase): ModeInfo[] {
  const mode = getGameModeDefinition(state.modeId);
  return (mode.infoFeatures ?? []).flatMap((id) => featureHandlers[id]?.(state, phase) ?? genericInfo(id, phase));
}
...
function genericInfo(id: InfoFeatureId, phase: Phase): ModeInfo[] {
  return fact(id, labelFromId(id), `active on ${phase}`, phase);
}
